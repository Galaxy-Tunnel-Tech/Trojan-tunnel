...
function vlessUri(host, uuid, path) {
...
async function handleTrojan(request, env, logger) {
  ...
  const establish = async (payload) => {
    ...
    const request = parseAddress(payload, reqStart + 3);
    if (!request || payload[reqStart] !== 1 || payload[reqStart + 1] !== 1) throw new Error("unsupported trojan request");
...
const worker_default = {
  async fetch(request, env) {
    ...
    if (url.pathname === "/sub" && request.method === "GET") {
      ...
      return new Response(btoa(vlessUri(request.headers.get("Host") || url.host, uuid, wsPath)), { headers: { ...headers("text/plain; charset=utf-8"), "Profile-Update-Interval": "24" } });
    }
